...
export default {
  ...
  focusBackToPane(pane) {
    ...
    if (currentSession) {
      console.log(currentSession.pty);
      if (!currentSession.pty.focusBackToPaneListener) {
        ...
        currentSession.pty.onData((data) => {
          console.log(data);
          console.log(done);
          // if (!done && (data.includes('kuma@Desktop') || data.includes('root@Desktop'))) {
          //   pane.activate();
          //   done = true;
          // }
          if (!done && (data.endsWith('[m$ ') || data.endsWith('[m: ') || data.endsWith('# '))) {
            console.log('################');
            pane.activate();
            done = true;
          }
